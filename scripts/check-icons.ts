...
/** Verify icon integrity across icons.ts, the SVG folder, the mapping files, and generator.ts. Throws on any mismatch. */
export function checkIcons(): void {
  ...
  // Check 3: icon IDs used in mapping files must be defined in icons.ts (also collect used IDs for Check 4)
  const mappingFiles = [
  ...
  for (const file of mappingFiles) {
    ...
    while ((m = directRegex.exec(content)) !== null) {
      ...
      if (!definedIds.has(m[1])) {
        undefinedRefs.push({ file, line: lineNumberAt(content, m.index), id: m[1] });
      }
    }
  ...
  if (failed) {
    throw new Error("Icon integrity check failed. See the issues above.");
  }
  console.log("All icon checks passed.");
}

// Run directly (`node scripts/check-icons.ts`), not when imported.
